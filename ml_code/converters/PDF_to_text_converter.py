...
output_path = file
#%%
# if the sum if row pixels is too small it will become a black line
def is_line(rowsum):
    if rowsum < 200000:
        return True
    else:
...
def block_location(row_list):
    ...
    return block_list
#%%
# input is the result from img_to_data and retrieve usable text
def clean_text(text):
    scan = text.split('\t')
    useful_text = []
    for i in range(11, len(scan), 11):
        useful_text += [scan[i]]
