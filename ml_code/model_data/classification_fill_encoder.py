...
from ml_code.classification_models.rfc_class import rfc_class
# query user
...
df, df_null, embedding_map = df_encoder(state='TX',
...
grid_search_obj = rfc_class(X_train, X_test, y_train, y_test)
prediction_df, predictions = add_pred(grid_search=grid_search_obj,
                         X_features=X_features, label='primary_merchant_name')
...
try:
    ...
    # one-element-tuple with (i, )
        cursor.execute(sql_insert_query, ([i.unique_mem_id, i.unique_bank_account_id,
                                           ...
                                           i.transaction_category_name, i.city,
                                           ...
                                           i.primary_merchant_name, i.merchants_pred,
                                           i.tr_essentiality, i.envelope_category]))

    connection.commit()
    print(len(feat_df), "record(s) inserted successfully.")
